...
public:
     ...
     * |          `solver_name`          |     `string`     |          Solver used to solve the problem. `ipopt` and `fatrop` (structure exploiting interior point, https://github.com/meco-group/fatrop) solve the nonlinear problem. `sqp` solves a sequence of convex QPs with osqp (Gauss-Newton), warm started with the previous solution; it is the fastest option.          |    Yes    |
     ...
     * |       `fatrop_tolerance`        |     `double`     |                                           Convergence tolerance of fatrop. Used only if `solver_name` is `fatrop` (default value is \f$10^{-8}\f$).                                           |     No    |
     * |    `number_of_qp_iterations`    |       `int`      | Used only if `solver_name` is `sqp`. Maximum number of QPs solved at each advance. With `1` the controller implements a real-time iteration scheme (default `1`). At the first advance the sqp runs until convergence. |     No    |
     * |         `sqp_tolerance`         |     `double`     |                        Used only if `solver_name` is `sqp`. The sqp stops when the infinity norm of the step is smaller than this value (default \f$10^{-4}\f$).                         |     No    |
     * |        `osqp_tolerance`         |     `double`     |                                   Used only if `solver_name` is `sqp`. Absolute and relative tolerance of osqp (default \f$10^{-5}\f$).                                    |     No    |
     * |      `osqp_max_iteration`       |       `int`      |                                       Used only if `solver_name` is `sqp`. Maximum number of iterations of osqp (default `4000`).                                       |     No    |
